...
class Metric:
    ...
    def create_metric_intrinsic_jobs(
        ...
        """Create jobs for a single input classification result folder (eg. ref or c1 or c2 or ...)
        These jobs are aimed to compute intermediate results relative to the input las only (without comparison) that
        will be used in `create_metric_relative_to_ref_jobs` to compute the relative metric
        Args:
            ...
            is_ref (bool): flag that says if the input classification folder is the reference folder (in case it has to
